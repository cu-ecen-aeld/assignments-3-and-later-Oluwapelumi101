...
/* ── Append packet and send back full file (mutex-protected) ────────────── */
...
static void *connection_thread(void *arg) {
    ...
    while (!g_exit_requested) {
        ...
        if (r == 0) break;
        if (r < 0) { if (errno == EINTR) continue; syslog(LOG_ERR, "recv: %s", strerror(errno)); break; }
...
static void *timestamp_thread(void *arg) {
    ...
    while (!g_exit_requested) {
        ...
        int df = open(DATAFILE, O_CREAT | O_WRONLY | O_APPEND, 0644);
        if (df != -1) {
            write(df, timebuf, strlen(timebuf));
            close(df);
        }
        pthread_mutex_unlock(&g_file_mutex);
    }
...
int main(int argc, char *argv[]) {
    ...
    while (!g_exit_requested) {
        ...
        struct conn_data *cd, *tmp;
        SLIST_FOREACH_SAFE(cd, &head, entries, tmp) {
    ...
    /* Shutdown: join all remaining threads */
    struct conn_data *cd, *tmp;
    SLIST_FOREACH_SAFE(cd, &head, entries, tmp) {
        pthread_join(cd->tid, NULL);
        SLIST_REMOVE(&head, cd, conn_data, entries);
